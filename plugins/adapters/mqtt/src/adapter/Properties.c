...
RTI_MQTT_MqttProtocolVersion_from_string(
    const char *str, RTI_MQTT_MqttProtocolVersion *proto_out)
{
...
RTI_MQTT_SslTlsProtocolVersion_from_string(
        const char *str, RTI_MQTT_SslTlsProtocolVersion *proto_out)
{
    if (RTI_MQTT_String_compare(str,"default") == 0 ||
...
static DDS_ReturnCode_t
RTI_MQTT_QosLevel_from_string(const char *str,
                                  RTI_MQTT_QosLevel *qos_out)
{
    ...
    else if (RTI_MQTT_String_compare(str,"one") == 0 ||
    ...
    else if (RTI_MQTT_String_compare(str,"two") == 0 ||
...
RTI_MQTT_PersistenceLevel_from_string(
    const char *str, RTI_MQTT_PersistenceLevel *level_out)
{
    ...
    else if (RTI_MQTT_String_compare(str,"durable") == 0 ||
...
DDS_StringSeq_from_string(const char *str, struct DDS_StringSeq *seq)
{
    ...
    while (consumed_len < str_len)
    {
        ...
        seq_val = (char *) 
                    RTI_MQTT_Heap_allocate(sizeof(char)*(cur_str_len+1));
        if (seq_val == NULL)
        {
            /* TODO Log error */
            goto done;
...
DDS_OctetSeq_from_string(const char *str, struct DDS_OctetSeq *seq)
{
    ...
    {
        RTI_MQTT_ERROR_1("failed to ensure octet sequence length:","%u",
                str_len+1)
        goto done;
    }
    ...
    if (retval != DDS_RETCODE_OK)
    {
        ...
        {
            RTI_MQTT_ERROR_1("failed to reset octet sequence length:","%u",
                orig_seq_len)
        }
    }
...
DDS_ReturnCode_t
DDS_Boolean_from_string(const char *str, DDS_Boolean *bool_out)
{
    ...
    else if (RTI_MQTT_String_compare(str,"f") == 0 ||
    ...
    RTI_RS_MQTT_BrokerConnectionConfig **config_out)
{
    ...
    if (!RTI_RS_MQTT_BrokerConnectionConfig_copy(
            config, &RTI_RS_MQTT_BrokerConnectionConfig_DEFAULT))
    {
        RTI_MQTT_LOG_COPY_DATA_FAILED(
            "RTI_RS_MQTT_BrokerConnectionConfig", config, &RTI_RS_MQTT_BrokerConnectionConfig_DEFAULT)
        goto done;
    }
    ...
    RTI_RS_MQTT_BrokerConnectionConfig **config_out)
{
    ...
    config = 
        RTI_RS_MQTT_BrokerConnectionConfigTypeSupport_create_data_w_params(
                                                            &alloc_params);
    if (config == NULL)
    {
        /* TODO Log error */
...
void
RTI_RS_MQTT_BrokerConnectionConfig_delete(
    ...
    RTI_MQTT_ClientConfig **config_out)
{
    ...
    RTI_RS_MQTT_lookup_property(properties, 
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        if (DDS_RETCODE_OK !=
                RTI_MQTT_MqttProtocolVersion_from_string(pval, &proto_v))
        {
            /* TODO Log error */
            goto done;
        }
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        struct DDS_StringSeq servers = DDS_SEQUENCE_INITIALIZER;
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        config->connect_timeout.nanoseconds = 
                    RTI_MQTT_String_to_long(pval,NULL,0);)
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        if (DDS_RETCODE_OK != 
                DDS_Boolean_from_string(pval,&config->clean_session))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        config->max_reply_timeout.seconds = 
                    RTI_MQTT_String_to_long(pval,NULL,0);)

    RTI_RS_MQTT_lookup_property(properties, 
        RTI_MQTT_PROPERTY_CLIENT_MAX_REPLY_TIMEOUT_NANOSECONDS,
        config->max_reply_timeout.nanoseconds = 
                    RTI_MQTT_String_to_long(pval,NULL,0);)

    RTI_RS_MQTT_lookup_property(properties,
        RTI_MQTT_PROPERTY_CLIENT_RECONNECT,
        if (DDS_RETCODE_OK != 
                DDS_Boolean_from_string(pval,&config->reconnect))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_lookup_property(properties, RTI_MQTT_PROPERTY_CLIENT_PASSWORD,
        ...
        {
            if (!DDS_OctetSeq_copy(config->password,&password))
            {
                RTI_MQTT_ERROR("failed to copy password sequence")
                failed = DDS_BOOLEAN_TRUE;
            }
            if (!DDS_OctetSeq_finalize(&password))
            {
                RTI_MQTT_ERROR("failed to finalize password sequence")
                failed = DDS_BOOLEAN_TRUE;
            }
        }
        else
        {
            config->password = (struct DDS_OctetSeq*)
                    RTI_MQTT_Heap_allocate(sizeof(struct DDS_OctetSeq));
            if (config->password == NULL)
            {
                RTI_MQTT_ERROR("failed to allocate password sequence")
                if (!DDS_OctetSeq_finalize(&password))
                {
                    RTI_MQTT_ERROR("failed to finalize password sequence")
                }
...
{\
    ...
    { \
        (c_)->ssl_tls_config = (RTI_MQTT_SslTlsConfig*) \
                RTI_MQTT_Heap_allocate(sizeof(RTI_MQTT_SslTlsConfig)); \
        if ((c_)->ssl_tls_config == NULL) \
        { \
            /* TODO Log error */ \
            goto done; \
        ...
        /* Set default value here. We might want to refactor this macro to \
         * it's own function (e.g. RTI_MQTT_SslTlsConfig_new) */ \
        (c_)->ssl_tls_config->verify_server_certificate = DDS_BOOLEAN_TRUE; \
    } \
}
    /* Scan server URIs for SSL URIS, and allocate a configuration if
       at least one is found */
    {
        DDS_UnsignedLong uris_len = 0,
                         i = 0;
        ...
        for (i = 0; i < uris_len && config->ssl_tls_config == NULL; i++)
        {
            ...
            if (uri ==
                    RTI_MQTT_String_find_substring(
                            uri, RTI_MQTT_URI_PREFIX_SSL))
            {
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        allocate_ssl_tls_config(config);
        DDS_String_replace(&config->ssl_tls_config->private_key, pval);
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        allocate_ssl_tls_config(config);
        DDS_String_replace(
            &config->ssl_tls_config->private_key_password, pval);
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        if (config->ssl_tls_config->cypher_suites == NULL)
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        if (DDS_RETCODE_OK != 
                RTI_MQTT_SslTlsProtocolVersion_from_string(
                    pval, &config->ssl_tls_config->protocol_version))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_lookup_property(properties,
        ...
        if (DDS_RETCODE_OK != 
                DDS_Boolean_from_string(
                    pval, &config->ssl_tls_config->verify_server_certificate))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_BrokerConnectionConfig **config_out)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_RS_MQTT_BrokerConnectionConfig_default(&config))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RS_MQTT_MessageReaderConfig **config_out)
{
    ...
    if (!RTI_RS_MQTT_MessageReaderConfig_copy(
                config, &RTI_RS_MQTT_MessageReaderConfig_DEFAULT))
    {
        RTI_MQTT_LOG_COPY_DATA_FAILED("RTI_RS_MQTT_MessageReaderConfig", 
            config, &RTI_RS_MQTT_MessageReaderConfig_DEFAULT)
        goto done;
    }
    ...
    RTI_RS_MQTT_MessageReaderConfig **config_out)
{
    ...
    config = 
        RTI_RS_MQTT_MessageReaderConfigTypeSupport_create_data_w_params(
                                                                &alloc_params);
    if (config == NULL)
    {
        /* TODO Log error */
...
void
RTI_RS_MQTT_MessageReaderConfig_delete(
    ...
    RTI_MQTT_SubscriptionConfig **config_out)
{
    ...
    RTI_RS_MQTT_lookup_property(properties, 
        ...
        DDS_Boolean failed = DDS_BOOLEAN_FALSE;
        if (DDS_RETCODE_OK !=
                DDS_StringSeq_from_string(pval, &topics))
        {
            /* TODO Log error */
            goto done;
        }
    ...
    RTI_RS_MQTT_MessageReaderConfig **config_out)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_RS_MQTT_MessageReaderConfig_default(&config))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RS_MQTT_MessageWriterConfig **config_out)
{
    ...
    if (!RTI_RS_MQTT_MessageWriterConfig_copy(
                config, &RTI_RS_MQTT_MessageWriterConfig_DEFAULT))
    {
        RTI_MQTT_LOG_COPY_DATA_FAILED("RTI_RS_MQTT_MessageWriterConfig", 
                config, &RTI_RS_MQTT_MessageWriterConfig_DEFAULT)
        goto done;
    }
    ...
    RTI_RS_MQTT_MessageWriterConfig **config_out)
{
    ...
    config = 
        RTI_RS_MQTT_MessageWriterConfigTypeSupport_create_data_w_params(
                                                                &alloc_params);
    if (config == NULL)
    {
        /* TODO Log error */
...
void
RTI_RS_MQTT_MessageWriterConfig_delete(
...
static
DDS_ReturnCode_t
RTI_MQTT_PublicationConfig_parse_from_properties(
    const struct RTI_RoutingServiceProperties * properties,
    RTI_MQTT_PublicationConfig **config_out)
{
    ...
    RTI_RS_MQTT_lookup_property(properties,
        ...
        if (DDS_RETCODE_OK != 
                DDS_Boolean_from_string(pval,&config->retained))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_lookup_property(properties,
        ...
        if (DDS_RETCODE_OK != 
                DDS_Boolean_from_string(pval,&config->use_message_info))
        {
            /* TODO Log error */
            goto done;
        })
    ...
    RTI_RS_MQTT_MessageWriterConfig **config_out)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_RS_MQTT_MessageWriterConfig_default(&config))
    {
        /* TODO Log error */
        goto done;
    }
