...
void
RTI_MQTT_ClientMqttApi_Paho_on_success(
    void *ctx, MQTTAsync_successData *response)
{
...
void
RTI_MQTT_ClientMqttApi_Paho_on_failure(
    void *ctx, MQTTAsync_failureData *response)
{
    struct RTI_MQTT_PendingRequest *req =
            (struct RTI_MQTT_PendingRequest*)ctx;
    ...
    struct RTI_MQTT_Client *self)
{
    ...
    if (client_addr == NULL || 
            RTI_MQTT_String_length(client_addr) == 0)
    {
    ...
    if (client_id == NULL || 
            RTI_MQTT_String_length(client_id) == 0)
    {
        RTI_MQTT_LOG_CLIENT_INVALID_CONFIG_DETECTED(
            self, "invalid client id specified")
        goto done;
    }
    ...
    case RTI_MQTT_PersistenceLevel_DURABLE:
        ...
        client_persistence_storage = 
            self->data->config->persistence_storage;
        break;
    ...
    RTI_MQTT_LOG_4("created MQTT client:",
    ...
    if (MQTTASYNC_SUCCESS != 
            MQTTAsync_setCallbacks(self->client,
                            self,
...
RTI_MQTT_ClientMqttApi_Paho_connect(struct RTI_MQTT_Client *self)
{
    ...
    {
        RTI_MQTT_TIME_TO_SECONDS_FAILED(
                    &self->data->config->keep_alive_period)
        goto done;
    }
    ...
    conn_opts.serverURIcount = DDS_StringSeq_get_length(
                                    &self->data->config->server_uris);
    ...
    default:
        RTI_MQTT_LOG_CLIENT_UNSUPPORTED_PROTOCOL_VERSION_DETECTED(
            self, self->data->config->protocol_version)
        goto done;
        break;
    }
    ...
    if (self->data->config->password != NULL) {
        if (DDS_RETCODE_OK != 
                RTI_MQTT_DDS_OctetSeq_to_string(self->data->config->password,
                                       (char**)&conn_opts.password))
        {
            RTI_MQTT_OCTET_SEQ_TO_STRING_FAILED(
    ...
    if (self->data->config->ssl_tls_config != NULL)
    {
        ...
        if (self->data->config->ssl_tls_config->private_key_password != NULL &&
            RTI_MQTT_String_length(
                self->data->config->ssl_tls_config->private_key_password) > 0)
        {
        ...
        if (self->data->config->ssl_tls_config->identity != NULL &&
            RTI_MQTT_String_length(
                    self->data->config->ssl_tls_config->identity) > 0)
        {
            ssl_opts.keyStore =
        ...
        if (self->data->config->ssl_tls_config->cypher_suites != NULL &&
            RTI_MQTT_String_length(
                    self->data->config->ssl_tls_config->cypher_suites) > 0)
        {
...
RTI_MQTT_ClientMqttApi_Paho_disconnect(struct RTI_MQTT_Client *self)
{
    ...
    if (MQTTASYNC_SUCCESS !=
            MQTTAsync_disconnect(self->client, &opts))
    {
    ...
    struct RTI_MQTT_PendingRequest *req)
{
    ...
    for (i = 0; i < seq_len; i++)
    {
        RTI_MQTT_SubscriptionParams *p =
            RTI_MQTT_SubscriptionParamsSeq_get_reference(&req_ctx->params, i);
    ...
    RTI_MQTT_TRACE_2("submit SUBSCRIPTIONS with Paho:","client=%p, subs=%d",
        self->client, seq_len)
    for (i = 0; i < seq_len; i++)
    {
    ...
    struct RTI_MQTT_PendingRequest *req)
{
...
#if RTI_MQTT_USE_TRACE
    RTI_MQTT_TRACE_2("cancel SUBSCRIPTIONS with Paho:","client=%p, subs=%d",
        self->client, seq_len)
    for (i = 0; i < seq_len; i++)
    {
    ...
    if (MQTTASYNC_SUCCESS !=
            MQTTAsync_unsubscribeMany(
                self->client, seq_len, sub_topics, &opts))
    {
        RTI_MQTT_LOG_CLIENT_PAHO_C_SUBSCRIBE_FAILED(self)
        goto done;
    }
    ...
    struct RTI_MQTT_PendingRequest *req)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_QosLevel_to_mqtt_qos(
                    params->qos_level,&async_msg.qos))
    {
    ...
    if (MQTTASYNC_SUCCESS !=
            MQTTAsync_sendMessage(
                    self->client, topic, &async_msg, &opts))
    {
...
void*
RTI_MQTT_ClientMqttApi_Paho_connection_lost_thread(void *arg)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_ClientMqttApi_create_client(self))
    {
...
void
RTI_MQTT_ClientMqttApi_Paho_on_connection_lost(void *ctx, char *cause)
{
    DDS_Boolean retval = DDS_BOOLEAN_FALSE,
                locked = DDS_BOOLEAN_FALSE;
...
int
RTI_MQTT_ClientMqttApi_Paho_on_message_arrived(
    void *ctx, char *topic_name, int topic_len, MQTTAsync_message *message)
{
    DDS_Boolean retval = DDS_BOOLEAN_FALSE;
    ...
    if (DDS_RETCODE_OK != 
                RTI_MQTT_QosLevel_from_mqtt_qos(
                        message->qos,&msg_info.qos_level))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    msg_info.duplicate = 
        (message->dup)? DDS_BOOLEAN_TRUE : DDS_BOOLEAN_FALSE;

    if (DDS_RETCODE_OK != 
            RTI_MQTT_Client_on_message_arrived(self, 
                                               topic_name,
                                               message->payload,
                                               message->payloadlen,
                                               &msg_info))
